let app = new Vue({
    ...
    methods: {
        ...
        getNumDataOLD(data) {
            ...
            let officeSpent = Math.floor(data.robot + data.oklad + data.office + nalog + salary)
            let officeSalary = Math.floor((data.summHold + data.diffHold) * 0.6 * 10)
            let total = Math.floor( officeSalary - officeSpent - this.getSalaryOLD(data.summHold).super - this.getSalaryOLD(data.summHold).trafic )
            return [nalog, salary, officeSpent, officeSalary, total]
        },
        getSalary(num) {
            let idx = 0
        ...
        getSalaryOLD(num) {
            ...
            for (let i = 0; i < this.ladderOLD.length; i++) {
                if (num >= this.ladderOLD[i][0]) {
                    idx += 1
                } else if (num < this.ladderOLD[i][0]) {
                    idx = i - 1 
                    break
                }
            }
            ...
            let salaryData = {
                "super" : this.ladderOLD[idx][1],
                "trafic" : this.ladderOLD[idx][2]
            }

            let salaryZero = {
                "super" : this.superOkladOLD,
                "trafic" : this.traficOkladOLD
            }
